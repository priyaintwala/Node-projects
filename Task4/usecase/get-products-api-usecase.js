    ...
    return async function getAllProductsUsecase() {
      try {
        ...
        while (currentPage < totalPages) {
          ...
          let aurl = `https://api.bigcommerce.com/stores/************8/v3/catalog/products?include=custom_fields,variants&limit=50&page=${currentPage}`;
       
  
          const nextPageResponse = await fetch(aurl, options);
          const nextPageData = await nextPageResponse.json();

          console.log("working fine till json");
